# Spanish dictionary backfill: translates every dictionary entry's gloss into
# natural Spanish (definitions.es) with Gemini, then mirrors it onto
# path_word_slots.es. Resumable: only rows with empty es are fetched.
# Mirror of tools/ru_backfill.py.
import json
import os
import sys
import time
...
def sql(query):
    req = urllib.request.Request(
        ...
        headers={"Authorization": f"Bearer {PAT}",
                 ...
                 "User-Agent": "Mozilla/5.0 (sinoma-es-backfill)"},
        method="POST")
...
PROMPT = (
    "You are a professional Chinese-Spanish lexicographer compiling a "
    "diccionario chino-español (Chinese-Spanish learner's dictionary) for "
    "Spanish native speakers studying Mandarin.\n"
    "For each entry below, write the Spanish dictionary gloss of the Chinese "
    "word. Rules:\n"
    "- Natural, idiomatic neutral/standard Spanish exactly as a published "
    "Spanish dictionary (RAE) would print it, with correct accents and ñ. "
    "Never translate word-by-word from English.\n"
    "- Citation form: nouns with their definite article to show gender "
    "(el libro, la casa), verbs as the infinitive (estudiar, leer), adjectives "
    "in masculine singular.\n"
    "- 1-3 senses, comma-separated, concise. No sentences, no explanations, "
    "no pinyin.\n"
    "- Particles/measure words/grammar words: give the standard Spanish "
    "linguistic description (e.g. 的 -> 'partícula posesiva/atributiva').\n"
    "- Match the part of speech given.\n"
    "Return ONLY a JSON object mapping each id to its Spanish gloss string.\n\n"
    "Entries:\n"
)
...
def main():
    ...
    while True:
        ...
        sql("update dictionary d set definitions = "
            "jsonb_set(coalesce(d.definitions,'{}'::jsonb),'{es}',"
            "to_jsonb(v.es)) "
            f"from (values {values}) as v(id, es) where d.id = v.id")
        total_done += len(ok)
        print(f"done {total_done} (+{len(ok)})", flush=True)
        time.sleep(4)
    sql("update path_word_slots s set es = d.definitions->>'es' "
        "from dictionary d where d.simplified = s.word "
        "and coalesce(d.definitions->>'es','') <> '' "
        "and coalesce(s.es,'') = ''")
    left = sql("select count(*) n from dictionary "
               "where coalesce(definitions->>'es','') = ''")
    slots = sql("select count(*) n from path_word_slots "
                "where coalesce(es,'') = ''")
    print(f"FINISHED. dictionary missing es: {left[0]['n']}, "
          f"slots missing es: {slots[0]['n']}")
