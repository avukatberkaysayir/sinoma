# One-off: add a French (11th) argument to every _t(...) call in
# locale_provider.dart. Parses each call's 10 string-literal args, translates the
# English (2nd) arg into natural UI French via the ko-batch Gemini proxy
# (preserving $placeholders, leading CJK/emoji prefixes and punctuation), then
# inserts the French literal before the closing paren. Caches translations in
# tools/_fr_locale_cache.json so re-runs don't re-hit Gemini.
# Mirror of tools/_pt_locale_fill.py (now expects 10 existing args).
...
CACHE = os.path.join(ROOT, "tools", "_fr_locale_cache.json")
MODELS = ["gemini-flash-lite-latest", "gemini-flash-latest",
          "gemini-2.5-flash", "gemini-2.5-flash-lite"]
NARGS = 10  # existing positional args before French is appended
    ...
    for _ in range(12):
        ...
        req = urllib.request.Request(
            ...
            headers={"Content-Type": "application/json",
                     ...
                     "User-Agent": "Mozilla/5.0 (sinoma-fr-locale)"}, method="POST")
        try:
...
PROMPT = (
    "You are a professional EN->FR translator localizing a Mandarin-learning "
    "app's UI. Translate each English string into natural, idiomatic standard "
    "French (the way a French app would phrase it; concise UI register, correct "
    "gender/agreement and accents). STRICT RULES:\n"
    "- Keep every $-placeholder EXACTLY as-is ($n, $d, $t, $h, $level, etc.), "
    "in a position that is grammatical in French.\n"
    "- Preserve any leading non-Latin prefix verbatim (e.g. Chinese '文法  ', "
    "'字数  ', '全部  '), all emojis (🧧 etc.), arrows, dots and punctuation.\n"
    "- Use proper French accents (é, è, ê, à, ç, ù, î, ô) and typography "
    "(apostrophe elision l', d', etc.).\n"
    "- Do NOT add quotes around the result. Do NOT translate placeholders.\n"
    "- Return ONLY a JSON object mapping each given id to its French string.\n\n"
    "Strings:\n")
...
def main():
    ...
    for start, end, args in sorted(good, key=lambda c: c[1], reverse=True):
        ...
        fr = cache[en]
        src = src[:end - 1] + f", '{esc(fr)}'" + src[end - 1:]
    open(TARGET, "w", encoding="utf-8", newline="\n").write(src)
    print(f"inserted fr into {len(good)} calls")
