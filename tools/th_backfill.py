# Thai dictionary backfill: translates every dictionary entry's gloss into
# natural Thai (definitions.th) with Gemini, then mirrors it onto
# path_word_slots.th. Resumable: only rows with empty th are fetched.
# Mirror of tools/vi_backfill.py.
import json
import os
import sys
import time
...
def sql(query):
    req = urllib.request.Request(
        ...
        headers={"Authorization": f"Bearer {PAT}",
                 ...
                 "User-Agent": "Mozilla/5.0 (sinoma-th-backfill)"},
        method="POST")
...
PROMPT = (
    "You are a professional Chinese-Thai lexicographer compiling a "
    "พจนานุกรมจีน-ไทย (Chinese-Thai learner's dictionary) for Thai "
    "native speakers studying Mandarin.\n"
    "For each entry below, write the Thai dictionary gloss of the Chinese "
    "word. Rules:\n"
    "- Natural, idiomatic standard Thai in Thai script (อักษรไทย) exactly as a "
    "published Thai dictionary (ราชบัณฑิตยสภา) would print it. NEVER use "
    "romanization/transliteration. Never translate word-by-word from English.\n"
    "- 1-3 senses, comma-separated, concise. No sentences, no explanations, "
    "no pinyin.\n"
    "- Verbs in plain form (เรียน, อ่าน, ไป); adjectives plain (ใหญ่, สวย); nouns "
    "as plain nouns with the right classifier sense where helpful.\n"
    "- Particles/measure words/grammar words: give the standard Thai "
    "linguistic description (e.g. 的 -> 'คำแสดงความเป็นเจ้าของ/คำขยายนาม').\n"
    "- Match the part of speech given.\n"
    "Return ONLY a JSON object mapping each id to its Thai gloss string.\n\n"
    "Entries:\n"
)
...
def main():
    ...
    while True:
        ...
        sql("update dictionary d set definitions = "
            "jsonb_set(coalesce(d.definitions,'{}'::jsonb),'{th}',"
            "to_jsonb(v.th)) "
            f"from (values {values}) as v(id, th) where d.id = v.id")
        total_done += len(ok)
    ...
    sql("update path_word_slots s set th = d.definitions->>'th' "
        "from dictionary d where d.simplified = s.word "
        "and coalesce(d.definitions->>'th','') <> '' "
        "and coalesce(s.th,'') = ''")
    left = sql("select count(*) n from dictionary "
               "where coalesce(definitions->>'th','') = ''")
    slots = sql("select count(*) n from path_word_slots "
                "where coalesce(th,'') = ''")
    print(f"FINISHED. dictionary missing th: {left[0]['n']}, "
          f"slots missing th: {slots[0]['n']}")
