# Lists active videos whose quiz JSON lacks Vietnamese options (vi.correctAnswer
# / vi.wrongAnswer). Output feeds the manual Vietnamese fill-in pass.
# Mirror of tools/list_missing_id_quiz.py.
...
select id,
       ...
       quiz->'vi'->>'correctAnswer'  as vi_c,
       quiz->'vi'->>'wrongAnswer'    as vi_w
from videos
where status='active'
...
missing = [r for r in rows
           if not (r.get('vi_c') or '').strip()
           or not (r.get('vi_w') or '').strip()]
print(f'active={len(rows)} missing_vi={len(missing)}')
for r in missing:
