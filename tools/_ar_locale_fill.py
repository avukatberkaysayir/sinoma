# One-off: add an Arabic (12th) argument to every _t(...) call in
# locale_provider.dart. Parses each call's 11 string-literal args, translates the
# English (2nd) arg into natural UI Arabic (Modern Standard Arabic, Arabic
# script) via the ko-batch Gemini proxy (preserving $placeholders, leading
# CJK/emoji prefixes and punctuation), then inserts the Arabic literal before the
# closing paren. Caches translations in tools/_ar_locale_cache.json.
# Mirror of tools/_fr_locale_fill.py (now expects 11 existing args).
...
CACHE = os.path.join(ROOT, "tools", "_ar_locale_cache.json")
MODELS = ["gemini-flash-lite-latest", "gemini-flash-latest",
          "gemini-2.5-flash", "gemini-2.5-flash-lite"]
NARGS = 11  # existing positional args before Arabic is appended
    ...
    for _ in range(12):
        ...
        req = urllib.request.Request(
            ...
            headers={"Content-Type": "application/json",
                     ...
                     "User-Agent": "Mozilla/5.0 (sinoma-ar-locale)"}, method="POST")
        try:
...
PROMPT = (
    "You are a professional EN->AR translator localizing a Mandarin-learning "
    "app's UI. Translate each English string into natural, idiomatic Modern "
    "Standard Arabic (MSA, فصحى) in ARABIC SCRIPT — the way a polished Arabic "
    "app would phrase it (concise UI register). STRICT RULES:\n"
    "- Write ONLY in Arabic script; NEVER romanization/transliteration.\n"
    "- Keep every $-placeholder EXACTLY as-is ($n, $d, $t, $h, $level, etc.), "
    "in a position that is grammatical in Arabic.\n"
    "- Preserve any leading non-Latin prefix verbatim (e.g. Chinese '文法  ', "
    "'字数  ', '全部  '), all emojis (🧧 etc.), arrows, dots and punctuation. Use "
    "the Arabic question mark ؟ and comma ، where natural.\n"
    "- Do NOT add quotes around the result. Do NOT translate placeholders.\n"
    "- Return ONLY a JSON object mapping each given id to its Arabic string.\n\n"
    "Strings:\n")
...
def main():
    ...
    for start, end, args in sorted(good, key=lambda c: c[1], reverse=True):
        ...
        ar = cache[en]
        src = src[:end - 1] + f", '{esc(ar)}'" + src[end - 1:]
    open(TARGET, "w", encoding="utf-8", newline="\n").write(src)
    print(f"inserted ar into {len(good)} calls")
