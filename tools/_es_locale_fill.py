# One-off: add a Spanish (9th) argument to every _t(...) call in
# locale_provider.dart. Parses each call's 8 string-literal args, translates the
# English (2nd) arg into natural UI Spanish via the ko-batch Gemini proxy
# (preserving $placeholders, leading CJK/emoji prefixes and punctuation), then
# inserts the Spanish literal before the closing paren. Caches translations in
# tools/_es_locale_cache.json so re-runs don't re-hit Gemini.
# Mirror of tools/_ru_locale_fill.py (now expects 8 existing args).
...
CACHE = os.path.join(ROOT, "tools", "_es_locale_cache.json")
MODELS = ["gemini-flash-lite-latest", "gemini-flash-latest",
          "gemini-2.5-flash", "gemini-2.5-flash-lite"]
NARGS = 8  # existing positional args before Spanish is appended
    ...
    for _ in range(12):
        ...
        req = urllib.request.Request(
            ...
            headers={"Content-Type": "application/json",
                     ...
                     "User-Agent": "Mozilla/5.0 (sinoma-es-locale)"}, method="POST")
        try:
...
PROMPT = (
    "You are a professional EN->ES translator localizing a Mandarin-learning "
    "app's UI. Translate each English string into natural, idiomatic Spanish "
    "(neutral/standard, the way a Spanish app would phrase it; concise UI "
    "register, correct gender/agreement and accents). STRICT RULES:\n"
    "- Keep every $-placeholder EXACTLY as-is ($n, $d, $t, $h, $level, etc.), "
    "in a position that is grammatical in Spanish.\n"
    "- Preserve any leading non-Latin prefix verbatim (e.g. Chinese '文法  ', "
    "'字数  ', '全部  '), all emojis (🧧 etc.), arrows, dots and punctuation.\n"
    "- Use proper Spanish punctuation incl. opening ¿ and ¡ for questions/"
    "exclamations.\n"
    "- Do NOT add quotes around the result. Do NOT translate placeholders.\n"
    "- Return ONLY a JSON object mapping each given id to its Spanish string.\n\n"
    "Strings:\n")
...
def main():
    ...
    for start, end, args in sorted(good, key=lambda c: c[1], reverse=True):
        ...
        es = cache[en]
        src = src[:end - 1] + f", '{esc(es)}'" + src[end - 1:]
    open(TARGET, "w", encoding="utf-8", newline="\n").write(src)
    print(f"inserted es into {len(good)} calls")
