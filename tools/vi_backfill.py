# Vietnamese dictionary backfill: translates every dictionary entry's gloss into
# natural Vietnamese (definitions.vi) with Gemini, then mirrors it onto
# path_word_slots.vi. Resumable: only rows with empty vi are fetched.
# Mirror of tools/id_backfill.py.
import json
import os
import sys
import time
...
def sql(query):
    req = urllib.request.Request(
        ...
        headers={"Authorization": f"Bearer {PAT}",
                 ...
                 "User-Agent": "Mozilla/5.0 (sinoma-vi-backfill)"},
        method="POST")
...
PROMPT = (
    "You are a professional Chinese-Vietnamese lexicographer compiling a "
    "từ điển Hoa-Việt (Chinese-Vietnamese learner's dictionary) for Vietnamese "
    "native speakers studying Mandarin.\n"
    "For each entry below, write the Vietnamese dictionary gloss of the Chinese "
    "word. Rules:\n"
    "- Natural, idiomatic standard Vietnamese exactly as a published Vietnamese "
    "dictionary would print it, with correct diacritics/tone marks. Never "
    "translate word-by-word from English.\n"
    "- 1-3 senses, comma-separated, concise. No sentences, no explanations, "
    "no pinyin.\n"
    "- Verbs in plain form (học, đọc, đi); adjectives plain (lớn, đẹp); nouns "
    "as plain nouns with the right classifier sense where helpful.\n"
    "- Particles/measure words/grammar words: give the standard Vietnamese "
    "linguistic description (e.g. 的 -> 'trợ từ sở hữu/định ngữ').\n"
    "- Match the part of speech given.\n"
    "Return ONLY a JSON object mapping each id to its Vietnamese gloss string.\n\n"
    "Entries:\n"
)
...
def main():
    ...
    while True:
        ...
        sql("update dictionary d set definitions = "
            "jsonb_set(coalesce(d.definitions,'{}'::jsonb),'{vi}',"
            "to_jsonb(v.vi)) "
            f"from (values {values}) as v(id, vi) where d.id = v.id")
        total_done += len(ok)
    ...
    sql("update path_word_slots s set vi = d.definitions->>'vi' "
        "from dictionary d where d.simplified = s.word "
        "and coalesce(d.definitions->>'vi','') <> '' "
        "and coalesce(s.vi,'') = ''")
    left = sql("select count(*) n from dictionary "
               "where coalesce(definitions->>'vi','') = ''")
    slots = sql("select count(*) n from path_word_slots "
                "where coalesce(vi,'') = ''")
    print(f"FINISHED. dictionary missing vi: {left[0]['n']}, "
          f"slots missing vi: {slots[0]['n']}")
