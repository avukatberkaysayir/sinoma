# Korean dictionary backfill: translates every dictionary entry's gloss into
# natural Korean (definitions.ko) with Gemini, then mirrors it onto
# path_word_slots.ko. Resumable: only rows with empty ko are fetched.
import json
import os
import sys
import time
...
def sql(query):
    req = urllib.request.Request(
        ...
        headers={"Authorization": f"Bearer {PAT}",
                 ...
                 "User-Agent": "Mozilla/5.0 (sinoma-ko-backfill)"},
        method="POST")
...
PROMPT = (
    "You are a professional Chinese-Korean lexicographer compiling a 중한사전 "
    "(Chinese-Korean learner's dictionary) for Korean native speakers studying "
    "Mandarin.\n"
    "For each entry below, write the Korean dictionary gloss of the Chinese "
    "word. Rules:\n"
    "- Natural, idiomatic Korean exactly as a published Korean dictionary "
    "would print it. Never translate word-by-word from English.\n"
    "- 1-3 senses, comma-separated, concise. No sentences, no explanations, "
    "no romanization, no hanja in parentheses unless essential.\n"
    "- Verbs end in -하다/-다 base form (e.g. 공부하다, 가다), adjectives in "
    "base form (크다, 예쁘다), nouns as plain nouns.\n"
    "- Particles/measure words/grammar words: give the standard Korean "
    "linguistic description (e.g. 的 -> '~의 (관형격 조사)').\n"
    "- Match the part of speech given.\n"
    "Return ONLY a JSON object mapping each id to its Korean gloss string.\n\n"
    "Entries:\n"
)
...
def main():
    ...
    while True:
        ...
        sql("update dictionary d set definitions = "
            "jsonb_set(coalesce(d.definitions,'{}'::jsonb),'{ko}',"
            "to_jsonb(v.ko)) "
            f"from (values {values}) as v(id, ko) where d.id = v.id")
        total_done += len(ok)
    ...
    sql("update path_word_slots s set ko = d.definitions->>'ko' "
        "from dictionary d where d.simplified = s.word "
        "and coalesce(d.definitions->>'ko','') <> '' "
        "and coalesce(s.ko,'') = ''")
    left = sql("select count(*) n from dictionary "
               "where coalesce(definitions->>'ko','') = ''")
    slots = sql("select count(*) n from path_word_slots "
                "where coalesce(ko,'') = ''")
    print(f"FINISHED. dictionary missing ko: {left[0]['n']}, "
          f"slots missing ko: {slots[0]['n']}")
