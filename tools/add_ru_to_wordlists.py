# Appends the Russian gloss as an 11th column to every word-list Dart file
# (hsk1..6_words.dart + diger_words.dart): [simplified, pinyin, pos, en, tr,
# ko, ja, id, vi, th, ru]. Russian comes from the dictionary table
# (definitions->>'ru'), which the Gemini backfill (ru_backfill.py) filled.
# Idempotent: rows that already have 11 columns are left alone.
# Mirror of tools/add_th_to_wordlists.py.
import json
import os
import re
import sys
...
def fetch_ru_map():
    ...
    off = 0
    while True:
        rows = sql("select simplified s, coalesce(definitions->>'ru','') k "
                   f"from dictionary order by id limit {page} offset {off}")
        for r in rows:
            if r["k"]:
                ru.setdefault(r["s"], r["k"])
        if len(rows) < page:
            break
        off += page
    return ru
...
def process(path, ru):
    ...
    for line in src.split("\n"):
        ...
        if count_cols(mid) >= 10:  # word excluded → already has ko..th+ru
            out.append(line)
            continue
    ...
    print(f"{os.path.basename(path)}: +ru on {changed} rows, "
          f"{missing} without a dictionary match")

def fix_header(path):
    src = open(path, encoding="utf-8").read()
    src = src.replace("[simplified, pinyin, pos, en, tr, ko, ja, id, vi, th]",
                      "[simplified, pinyin, pos, en, tr, ko, ja, id, vi, th, ru]")
    open(path, "w", encoding="utf-8", newline="\n").write(src)

def main():
    ru = fetch_ru_map()
    print(f"dictionary ru entries: {len(ru)}")
