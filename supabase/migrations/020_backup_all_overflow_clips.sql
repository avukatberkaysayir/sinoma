-- 020_backup_all_overflow_clips.sql
-- Rule (Berkay 2026-07-16): every active clip must sit in a slot. A clip that
-- maps to a curriculum slot but finds no FREE slot at its level must be marked
-- BACKUP — never left active-placeless. Multiple backup clips per slot are now
-- allowed, and the active/pending status of the slot holder no longer gates the
-- backup mark.
--
-- Changes vs 017/018: case (0) [exact clip already active] and case (c) [no free
-- slot] previously backup-marked only when (a) the primary slot was ACTIVE-held
-- AND (b) no other backup existed for it. Both guards are removed, so overflow
-- clips (3rd, 4th, … for the same slot) get a backup mark instead of silently
-- falling through to active-placeless. The batch then keeps any clip that did
-- not get a real level (placement) in "pending" rather than activating it.
  ...
  IF do_place THEN
    ...
    IF maxlvl IS NOT NULL THEN
      ...
      IF EXISTS (SELECT 1 FROM public.videos v2
        ...
        -- would-be primary slot (first matching grammar at this level, else word).
        -- Always mark (multiple backups per slot allowed).
      ...
      ELSE
        ...
        ELSE
          ...
          ELSE
            -- (c) no FREE slot → BACKUP at the primary would-be slot (first grammar
            -- at this level, else first word). Always mark — the holder may be
            -- active OR pending, and any number of backups may share the slot — so
            -- an overflow clip is NEVER left active-placeless.
            SELECT gl.unit AS unit, gl.name AS name INTO rec_g
              FROM unnest(COALESCE(NEW.quiz_categories, '{}'::text[])) WITH ORDINALITY t(cat, ord)
              JOIN public.grammar_levels gl ON gl.name = t.cat AND gl.level = maxlvl
              ORDER BY t.ord LIMIT 1;
