-- The SAME physical clip (youtube_id + start_time + end_time) can be ACTIVE in
-- only one slot. Re-importing a video whose clips were already activated created
-- a second PENDING row for the same clip, placed on a DIFFERENT slot (e.g. the
-- 很高兴认识你 clip was active on word 高兴 yet showed as a fresh pending placement
-- on grammar 很, with no "Yedeğe Al"). Slot occupancy didn't catch it because the
-- two rows sit on different slots.
--
-- Fix: at placement time, if this exact clip is already ACTIVE elsewhere, treat
-- this row as a backup of its would-be primary slot (so the admin sees "Yedeğe Al"
-- — it's a spare of an already-used clip), never a fresh active placement.
    ...
    IF maxlvl IS NOT NULL THEN
      ...
      ELSE
        ...
        ELSE
          ...
          ELSE
            -- (c) no FREE slot → backup ONLY if the clip's PRIMARY would-be slot is
            -- held by an ACTIVE clip; pending-only conflict leaves it UNPLACED.
            SELECT gl.unit AS unit, gl.name AS name INTO rec_g
              FROM unnest(COALESCE(NEW.quiz_categories, '{}'::text[])) WITH ORDINALITY t(cat, ord)
              JOIN public.grammar_levels gl ON gl.name = t.cat AND gl.level = maxlvl
              ORDER BY t.ord LIMIT 1;
