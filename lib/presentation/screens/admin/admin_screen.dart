  ...
  Future<void> _fillOtherLangs() async {
    ...
    try {
      ...
      if (mounted) {
        ...
        if ((t['ja'] ?? '').isNotEmpty) _jaCtrl.text = t['ja']!;
        if ((t['id'] ?? '').isNotEmpty) _idCtrl.text = t['id']!;
        if ((t['vi'] ?? '').isNotEmpty) _viCtrl.text = t['vi']!;
            ...
            Row(children: [
              ...
              Expanded(
                child: TextField(
                  ...
                  style: TextStyle(
                      color: AppColors.onSurface, fontSize: 13),
                  decoration: _dec('Endonezce anlam'),
                ),
              ),
              const SizedBox(width: 8),
              Expanded(
                child: TextField(
                  ...
                  style: TextStyle(
                      color: AppColors.onSurface, fontSize: 13),
                  decoration: _dec('Vietnamca anlam'),
                ),
              ),
              const SizedBox(width: 8),
              Expanded(
...
class _VideoCardState extends ConsumerState<_VideoCard> {
  ...
  late final TextEditingController _correctCtrlJa;
  late final TextEditingController _wrongCtrlJa;
  late final TextEditingController _correctCtrlId;
  late final TextEditingController _wrongCtrlId;
  late final TextEditingController _correctCtrlVi;
  late final TextEditingController _wrongCtrlVi;
  late final TextEditingController _correctCtrlTh;
  late final TextEditingController _wrongCtrlTh;
  late final TextEditingController _correctCtrlRu;
  ...
  late final TextEditingController _correctCtrlFr;
  late final TextEditingController _wrongCtrlFr;
  late final TextEditingController _correctCtrlAr;
  late final TextEditingController _wrongCtrlAr;
  ...
  void initState() {
    ...
    final quizJa = (quiz['ja'] as Map<String, dynamic>?) ?? {};
    _correctCtrlJa = TextEditingController(
        text: quizJa['correctAnswer'] as String? ?? '');
    _wrongCtrlJa = TextEditingController(
        text: quizJa['wrongAnswer'] as String? ?? '');
    final quizId = (quiz['id'] as Map<String, dynamic>?) ?? {};
    _correctCtrlId = TextEditingController(
        text: quizId['correctAnswer'] as String? ?? '');
    _wrongCtrlId = TextEditingController(
        text: quizId['wrongAnswer'] as String? ?? '');
    final quizVi = (quiz['vi'] as Map<String, dynamic>?) ?? {};
    _correctCtrlVi = TextEditingController(
        text: quizVi['correctAnswer'] as String? ?? '');
    _wrongCtrlVi = TextEditingController(
        text: quizVi['wrongAnswer'] as String? ?? '');
    final quizTh = (quiz['th'] as Map<String, dynamic>?) ?? {};
    _correctCtrlTh = TextEditingController(
        text: quizTh['correctAnswer'] as String? ?? '');
    _wrongCtrlTh = TextEditingController(
        text: quizTh['wrongAnswer'] as String? ?? '');
    final quizRu = (quiz['ru'] as Map<String, dynamic>?) ?? {};
    _correctCtrlRu = TextEditingController(
        text: quizRu['correctAnswer'] as String? ?? '');
    _wrongCtrlRu = TextEditingController(
        text: quizRu['wrongAnswer'] as String? ?? '');
    final quizEs = (quiz['es'] as Map<String, dynamic>?) ?? {};
    _correctCtrlEs = TextEditingController(
    ...
    final quizFr = (quiz['fr'] as Map<String, dynamic>?) ?? {};
    _correctCtrlFr = TextEditingController(
        text: quizFr['correctAnswer'] as String? ?? '');
    _wrongCtrlFr = TextEditingController(
        text: quizFr['wrongAnswer'] as String? ?? '');
    final quizAr = (quiz['ar'] as Map<String, dynamic>?) ?? {};
    _correctCtrlAr = TextEditingController(
        text: quizAr['correctAnswer'] as String? ?? '');
    _wrongCtrlAr = TextEditingController(
        text: quizAr['wrongAnswer'] as String? ?? '');
  ...
  void dispose() {
    ...
    _correctCtrlJa.dispose();
    _wrongCtrlJa.dispose();
    _correctCtrlId.dispose();
    _wrongCtrlId.dispose();
    _correctCtrlVi.dispose();
    _wrongCtrlVi.dispose();
    _correctCtrlTh.dispose();
    _wrongCtrlTh.dispose();
    _correctCtrlRu.dispose();
    _wrongCtrlRu.dispose();
    _correctCtrlEs.dispose();
    _wrongCtrlEs.dispose();
    _correctCtrlPt.dispose();
    _wrongCtrlPt.dispose();
    _correctCtrlFr.dispose();
    _wrongCtrlFr.dispose();
    _correctCtrlAr.dispose();
    _wrongCtrlAr.dispose();
    super.dispose();
  }
  ...
  Future<void> _generateQuiz() async {
    ...
    if (pivot &&
        (_correctCtrlEn.text.trim().isEmpty ||
            _wrongCtrlEn.text.trim().isEmpty ||
            !_enApproved)) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text(
              'Önce EN sekmesinde doğru VE yanlış İngilizce şıkkı üretip "İngilizce’yi Onayla"ya bas.')));
      return;
    }
    ...
    try {
      await widget.service.patchVideoFields(id, {
        ...
        'quiz': {
          ...
          'ja': {
            'correctAnswer': _correctCtrlJa.text.trim(),
            'wrongAnswer': _wrongCtrlJa.text.trim(),
          },
          'id': {
            'correctAnswer': _correctCtrlId.text.trim(),
            'wrongAnswer': _wrongCtrlId.text.trim(),
          },
          'vi': {
            'correctAnswer': _correctCtrlVi.text.trim(),
            'wrongAnswer': _wrongCtrlVi.text.trim(),
          },
          'th': {
            'correctAnswer': _correctCtrlTh.text.trim(),
            'wrongAnswer': _wrongCtrlTh.text.trim(),
          },
          'ru': {
            'correctAnswer': _correctCtrlRu.text.trim(),
            'wrongAnswer': _wrongCtrlRu.text.trim(),
          },
          'es': {
          ...
          'fr': {
            'correctAnswer': _correctCtrlFr.text.trim(),
            'wrongAnswer': _wrongCtrlFr.text.trim(),
          },
          'ar': {
            'correctAnswer': _correctCtrlAr.text.trim(),
            'wrongAnswer': _wrongCtrlAr.text.trim(),
          },
        },
      });
              ...
              child: Row(
                ...
                children: [
                  ...
                  if (_selectedQuizLang == 'tr') ...[
                    ...
                    _editField(_correctCtrlJa, 'Doğru cevap (JA)'),
                    _editField(_wrongCtrlJa, 'Yanlış cevap — tuzak (JA)'),
                  ] else if (_selectedQuizLang == 'id') ...[
                    _editField(_correctCtrlId, 'Doğru cevap (ID)'),
                    _editField(_wrongCtrlId, 'Yanlış cevap — tuzak (ID)'),
                  ] else if (_selectedQuizLang == 'vi') ...[
                    _editField(_correctCtrlVi, 'Doğru cevap (VI)'),
                    _editField(_wrongCtrlVi, 'Yanlış cevap — tuzak (VI)'),
                  ] else if (_selectedQuizLang == 'th') ...[
                    _editField(_correctCtrlTh, 'Doğru cevap (TH)'),
                    _editField(_wrongCtrlTh, 'Yanlış cevap — tuzak (TH)'),
                  ] else if (_selectedQuizLang == 'ru') ...[
                    _editField(_correctCtrlRu, 'Doğru cevap (RU)'),
                    _editField(_wrongCtrlRu, 'Yanlış cevap — tuzak (RU)'),
                    ...
                    _editField(_correctCtrlFr, 'Doğru cevap (FR)'),
                    _editField(_wrongCtrlFr, 'Yanlış cevap — tuzak (FR)'),
                  ] else if (_selectedQuizLang == 'ar') ...[
                    _editField(_correctCtrlAr, 'Doğru cevap (AR)'),
                    _editField(_wrongCtrlAr, 'Yanlış cevap — tuzak (AR)'),
                  ] else ...[
                    // Editing the English pair revokes approval, so target
                    // languages are always translated from a freshly-approved
                    // English (correct + wrong) — never a stale version.
                    _editField(_correctCtrlEn, 'Correct answer (EN)',
                        onChanged: (_) {
                      if (_enApproved) setState(() => _enApproved = false);
                    }),
                    ...
                    SizedBox(
                      width: double.infinity,
                      child: FilledButton.icon(
                        // Require BOTH English correct AND wrong before
                        // approval, so every target language translates the
                        // same complete English pair (no invented distractor).
...
class _LearnDesignTabState extends ConsumerState<_LearnDesignTab> {
  ...
  Widget _preview(String? url, String? fallbackAsset,
      {double h = 80, Widget? composed}) {
    Widget child;
